...
def zest_polarizability_bond_total():
    m = loprop.MolFrag(tmpdir, pf=mcpf, sf=mcsf)
    dAab = m.dAab
    dA = dAab.sum(axis=3).sum(axis=2).view(full.matrix)
    print dA
    assert True
...
# polarizabilities
   ...
   -0.11493635, -0.00016407,  0.45770123
    ...
    ihff = 1/(2*ff)
    Oxx = -ihff*(rMP[2][1] - rMP[1][1])
    Oyy = -ihff*(rMP[4][2] - rMP[3][2])
    Ozz = -ihff*(rMP[6][3] - rMP[5][3])
