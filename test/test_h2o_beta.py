...
# modify Gagliardi penalty function to include unit conversion bug
    m = loprop.MolFrag(tmpdir, freqs=(0.0,), pf=penalty_function(2.0/xtang**2))
    ff = ref.ff
...
def test_total_charge_shift2():
    d2Q = m.d2Qa[0].sum(axis=0).view(full.matrix)
    d2Qref = [0., 0., 0., 0., 0., 0.]
    assert_(d2Qref, d2Q)
...
def test_bond_charge_shift_sum2():
    d2Qa  = m.d2Qab[0].sum(axis=1).view(full.matrix)
    d2Qaref = m.d2Qa[0]
    assert_(d2Qa, d2Qaref)
