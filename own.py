...
class Loprop:
    ...
    def __init__(self, opts):
        ...
        if not os.path.isdir( opts.tmpdir):
            print "%s: Directory not found: %s" % (sys.argv[0], opts.tmpdir)
            raise SystemExit
        import tarfile
        if opts.daltgz:
            tgz = tarfile.open( opts.daltgz, 'r:gz')
            tgz.extractall(path= opts.tmpdir)
        
        if opts.freqs:
            freqs = map(float, opts.freqs.split())
        else:
            freqs = (0.0, )
        needed_files = ["AOONEINT", "DALTON.BAS", "SIRIFC", "AOPROPER", "RSPVEC"]
        for file_ in needed_files:
            df = os.path.join( opts.tmpdir, file_)
            if not os.path.isfile( df ):
                print "%s does not exists" % ( df )
                print "Needed Dalton files to run loprop.py:"
                print "\n".join(needed_files)
                raise SystemExit
        else:
            gc = None
        if opts.gc is not None: 
            #Gauge center
            try:
                #gc = map(float, o.gc.split())
                gc = [float(i) for i in opts.gc.split()]
            except(ValueError):
                sys.stderr.write("Gauge center incorrect:%s\n" % opts.gc)
                sys.exit(1)
        self.molfrag = MolFrag(
            opts.tmpdir, pf = self.penalty_function(opts.alpha), gc=opts.gc, freqs=opts.freqs
            )
    def penalty_function(self, alpha=2):
        ...
        return pf
    def getPot( self ):
        return self.molfrag.output_potential_file(
        self.opts.max_l, self.opts.pol, self.opts.bc, self.opts.angstrom
        )
    def pairs(n):
...
def main():
    ...
    print lo.getPot()
    if o.verbose:
       lo.molfrag.output_by_atom(fmt="%9.5f", max_l=o.max_l, pol=o.pol, bond_centers=o.bc)
    print t
