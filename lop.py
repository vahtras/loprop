...
class Loprop:
    ...
    """
    A modified version of Molfrag.
    Initialized with a path to a directory containing DALTON output files:
    ( defaults to /tmp )
    ...
    set_args( int max_l , int pol , int B )
        Set maximum angular momentum max_l ; Default = 0 monopole
        Set polarizability alpha ; Default = 1 isotropic
        Set hyperpolarizability beta ; Default = 0 none (not yet implemented for get_pot)


    get_pot()
        Returns a string representation of the potential file
        


    Example usage in other python package:

    from lop import Loprop

    output = Loprop( os.getcwd() ).get_pot()
    ...
    @staticmethod
    def pf( Za, Ra, Zb, Rb, alpha = 2 ):
        """Inverse half of penalty function defined in Gagliardi"""

        from math import exp
        ra = Loprop.rbs[int(round(Za))]
        rb = Loprop.rbs[int(round(Zb))]
    ...
    def __init__(
        self, tmpdir = "/tmp" , freqs=None, gc=None
        ):
...
#
    ...
    def get_pot( self ):
        return self.output_potential_file(
        self.Args.max_l, self.Args.pol, self.Args.bc, self.Args.angstrom
        )
    def set_args(self, max_l, pol, B ):
        self.Args.max_l = max_l
        self.Args.pol = pol
        self.Args.B = B
